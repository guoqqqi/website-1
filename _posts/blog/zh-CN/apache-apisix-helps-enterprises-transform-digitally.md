...
随着互联网、大数据、云计算、人工智能、物联网、区块链等一系列技术创新与行业服务的加速融合，行业产品创新能力正在不断提升，数字化、智能化不断催生新兴的服务模式和新产品，已成为企业数字化转型升级的新方向。证券企业加强核心技术自主可控能力，综合利用分布式与集中式相结合的技术路线，构建开放平台核心证券系统，并通过产品或服务输出，共享技术红利。

企业用户采购 API 网关作为企业自身数字化转型的一环，为后续持续转型和升级做铺垫，逐步向“全面云化”、“分布式化”、“智能化”、“开放化”的新技术体系演进。云原生 API 网关 Apache APISIX（以下简称 APISIX），可以完美的助力企业完成 API 管理和微服务架构转型。此外基于开源 APISIX 还有 k8s Ingress Controller 和 Service Mesh 产品，企业用户后续升级到 Kubernetes 和服务网格，提供统一、顺滑的过渡方案。底层技术栈的统一，极大降低学习、管理维护成本。
...
| ------------------------- | :----------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **控制台**                | 支持操作人员在控制台上操作，完成 API 管理、监控等需要                    | [APISIX](https://github.com/apache/apisix) 的开源版本自带[控制台](https://github.com/apache/apisix-dashboard)，也是 Apahce 顶级项目。[API7](https://www.apiseven.com/zh/apisix-vs-api7) 在开源 APISIX 基础上增加了用户、角色、工作区、多集群等大企业比较关心的额外功能。                                                                                                                                                                                                 |
| **动态路由、上游**        | 动态添加修改路由、上游                                                   | 在 APISIX 中路由、上游的添加修改是原生动态的，内存生效不需要任何进程替换，相比 Nginx 等需要 reload 的实现方案要好，不会产生流量抖动。                                                                                                                                                                                                                                                                                     |
| **负载均衡器**            | 反向代理支持指定不同负载均衡算法                                         | 除了已支持带有权重的 roundrobin 和一致性哈希算法外，还支持指数加权移动平均法（EWMA）和最小连接数算法。[APISIX](https://github.com/apache/apisix) 是目前唯一支持用户自定义负载均衡策略的 API 网关，满足企业用户的一些特殊场景需求。                                                                                                                                                                                        |
| **限流、限速熔断**        | 为保护上游对请求流量做频度控制                                           | 支持单机和集群两种方式限流限速。单机主要有三种：<br>1. limit-count：指定时间限制请求总数。<br>2. limit-req：限制请求频率。<br>3. limit-conn：限制并发数。<br>集群支持两种：<br>limit-count 和 limit-conn。                                                                                                                                                                                                                |
...
| **多集群管理**            | 管理控制台统一登录，同时管理不同集群的 API                               | 基于 APISIX 的商业版 [API7](https://www.apiseven.com/zh/apisix-vs-api7) 支持多集群：不同集群有完全独立的 API 和资源。企业不同主要业务线，为了避免相互干扰，优先推荐集群方式隔离。                                                                                                                                                                                                                                                  |
...
| **swagger 格式 API 导入** | 使用 [swagger](https://swagger.io) 标准完成 API 批量导入                 | 已内置支持 [OpenAPI Specification3.0](https://swagger.io/specification) 方式导入 [swagger](https://swagger.io) 。<br>但不同公司在使用 [swagger](https://swagger.io) API 格式文件时，会在注解中使用特定语义，通常这块需要适当定制开发。                                                                                                                                                                                    |

## 关于 Apache APISIX

**1.Apache 顶级项目**

APISIX 作为 Apache 顶级项目，中国**最快**毕业的顶级项目，Apache 唯一的 API 网关产品，也被纳入 CNCF 全景图。

![1.jpg](https://static.apiseven.com/logo/20210203/1.png)

目前很多流行的 AI、大数据类开源项目，主要集中在 Apache 基金会（300 多顶级项目）。APISIX 与这些项目具有相同的文化认同，更容易完成生态对接（如下图）。

![2.jpg](https://static.apiseven.com/logo/20210203/2.png)

**2.CNCF 基金会生态**

APISIX 的配置中心使用 etcd ，监控 prometheus ，链路追踪 SkyWalking 等技术选型与云原生技术栈吻合，降低用户迁到云原生体系成本。

![3.jpg](https://static.apiseven.com/logo/20210203/3.png)
...
![4.jpg](https://static.apiseven.com/logo/20210203/4.png)
...
![5.jpg](https://static.apiseven.com/logo/20210203/5.png)
...
![6.jpg](https://static.apiseven.com/logo/20210203/6.png)
...
![7.jpg](https://static.apiseven.com/logo/20210203/7.png)

对非常关心延迟指标的金融、证券行业，也在生产环境使用了 APISIX，比如前面提到的[空中云汇](https://www.airwallex.com/cn)等。

**9.APISIX core 维护**

Nginx 社区已经不活跃（开发、维护者偏少），发展滞后已严重跟不上 APISIX 的高速发展。所以 [API7](https://www.apiseven.com/zh/apisix-vs-api7) 使用了私有维护的 Nginx 分支，除了添加一些企业用户的必要新功能外（mTLS 双向认证、Dubbo、动态代理缓存等），也增强了性能、安全等方面。

**10.国产可信开源**

APISIX 在 2020 年 10 月份通过中国信息通信研究院和云计算开源产业联盟联合的可信开源评估，更多信息点击查看：[https://www.apiseven.com/zh/cert/trusted-cloud](https://www.apiseven.com/zh/cert/trusted-cloud) 。

## 关于支流科技公司

**1.团队成员**

- Apache APISIX 创始人、PMC 以及 Committor
- Apache Skywalking Committor
- OpenResty（Nginx + Lua）维护者
- 《OpenResty 最佳实践》、《OpenResty 从入门到实践》作者

**2.融资背景**

支流科技在半年时间内，完成了数千万美元的三轮融资，投资方为真格基金和真成投资

真格基金：[http://www.zhenfund.com](http://www.zhenfund.com)

真成投资：[http://www.zhenchengcap.com](http://www.zhenchengcap.com)

**3.本地优势**

快速和专业的处理，对于证券这种对于稳定性和实时性要求严苛的客户而言，尤为重要。

支流科技总部和创始团队都在深圳，遇到任何技术难题或者故障，可以得到分钟级别的响应，并由深圳本地的原厂技术团队直接解决。

## APISIX 助力企业数字化转型

**1.Kubernetes Ingress controller**

![9.jpg](https://static.apiseven.com/logo/20210203/9.png)
...
服务网格是最近两年热度比较高的部署方案。目前最流行的方案是 istio + envoy 的组合。APISIX 支持 xDS 协议，可以与 istio 一起工作，APISIX 与 envoy 二者平行存在可以互相替换。

![10.jpg](https://static.apiseven.com/logo/20210203/10.png)

**3.架构演变**

下图是后端服务架构演变史，APISIX 助力企业完成数字化转型，统一技术栈，降低企业开发、维护成本。开源，不锁定企业用户，建立公开、透明的 IT 市场，让企业用户专注于业务需求自身。

![11.jpg](https://static.apiseven.com/logo/20210203/11.png)
